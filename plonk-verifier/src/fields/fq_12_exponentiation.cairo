...
#[inline(always)]
fn addchain_exp_by_neg_t(x: Fq12, field_nz: NonZero<u256>, m: CircuitModulus) -> Fq12 {
    ...
    let t2 = x.mul(t0); // Step 5: t2 = x^0x11
    let t0 = t3.mul(t2); // Step 6: t0 = x^0x13
    let t1 = x.mul(t0); // Step 7: t1 = x^0x14
    let t4 = z.mul(t2); // Step 8: t4 = x^0x19
    let t6 = t2.cyclotomic_sqr_circuit(field_nz); // Step 9: t6 = x^0x22
    let t1 = t0.mul(t1); // Step 10: t1 = x^0x27
    let t0 = t3.mul(t1); // Step 11: t0 = x^0x29
    let t6 = t6.sqr_6_times(field_nz, m); // Step 17: t6 = x^0x880
    let t5 = t5.mul(t6); // Step 18: t5 = x^0x884
    let t5 = t4.mul(t5); // Step 19: t5 = x^0x89d
    let t5 = t5.sqr_7_times(field_nz, m); // Step 26: t5 = x^0x44e80
    let t4 = t4.mul(t5); // Step 27: t4 = x^0x44e99
    let t4 = t4.sqr_8_times(field_nz, m); // Step 35: t4 = x^0x44e9900
    let t4 = t0.mul(t4); // Step 36: t4 = x^0x44e9929
    let t3 = t3.mul(t4); // Step 37: t3 = x^0x44e992b
    let t3 = t3.sqr_6_times(field_nz, m); // Step 43: t3 = x^0x113a64ac0
    let t2 = t2.mul(t3); // Step 44: t2 = x^0x113a64ad1
    let t2 = t2.sqr_8_times(field_nz, m); // Step 52: t2 = x^0x113a64ad100
    let t2 = t0.mul(t2); // Step 53: t2 = x^0x113a64ad129
    let t2 = t2.sqr_6_times(field_nz, m); // Step 59: t2 = x^0x44e992b44a40
    let t2 = t0.mul(t2); // Step 60: t2 = x^0x44e992b44a69
    let t2 = t2.sqr_10_times(field_nz, m); // Step 70: t2 = x^0x113a64ad129a400
    let t1 = t1.mul(t2); // Step 71: t1 = x^0x113a64ad129a427
    let t1 = t1.sqr_6_times(field_nz, m); // Step 77: t1 = x^0x44e992b44a6909c0
    let t0 = t0.mul(t1); // Step 78: t0 = x^0x44e992b44a6909e9
    let z = z.mul(t0); // Step 79: z = x^0x44e992b44a6909f1
...
impl Fq12Exponentiation of PairingExponentiationTrait {
        ...
        loop {
            match naf.pop_front() {
                Option::Some(naf) => {
                    ...
                    if naf0 {
                        if naf1 {
                            result = FieldOps::mul(result, temp_sq);
                        } else {
                            result = FieldOps::mul(result, temp_sq.conjugate());
                        }
                    }

                    temp_sq = temp_sq.cyclotomic_sqr(field_nz, m);
                },
                Option::None => { break; },
            }
        };
    ...
    #[inline(always)]
    fn exp_by_neg_t(self: Fq12, field_nz: NonZero<u256>, m: CircuitModulus) -> Fq12 {
        addchain_exp_by_neg_t(self, field_nz, m)
    }
    ...
    fn final_exponentiation_easy_part(self: Fq12) -> Fq12 {
        ...
        let self = self.conjugate().div(self);
        // Software Implementation of the Optimal Ate Pairing
        // Page 9, 4.2 Final exponentiation
    ...
    fn final_exponentiation(self: Fq12, m: CircuitModulus) -> Fq12 {
        ...
        self.final_exponentiation_easy_part().final_exponentiation_hard_part(field_nz, m)
    }
    ...
    #[inline(always)]
    fn final_exponentiation_hard_part(self: Fq12, field_nz: NonZero<u256>, m: CircuitModulus) -> Fq12 {
        internal::revoke_ap_tracking();
        let a = self.exp_by_neg_t(field_nz, m);
        let b = a.cyclotomic_sqr_circuit(field_nz);
        let c = b.cyclotomic_sqr_circuit(field_nz);
        let d = c.mul(b);

        let e = d.exp_by_neg_t(field_nz, m);
        let f = e.cyclotomic_sqr_circuit(field_nz);
        let g = f.exp_by_neg_t(field_nz, m);
        let h = d.conjugate();
        let i = g.conjugate();
