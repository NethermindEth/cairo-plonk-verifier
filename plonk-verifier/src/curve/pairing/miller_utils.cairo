...
trait LineEvaluationsTrait<P1, P2> {
    ...
    fn at_tangent(self: P1, p: P2, m: CircuitModulus) -> Fq12;
    /// The sloped line function for adding two points
    fn at_chord(self: P1, p1: P2, p2: P2, m: CircuitModulus) -> Fq12;
}

impl G2LineEvals of LineEvaluationsTrait<AffineG2, AffineG1> {
    /// The sloped line function for doubling a point
    #[inline(always)]
    fn at_tangent(self: AffineG2, p: AffineG1, m: CircuitModulus) -> Fq12 {
        // -3px^2
        let cx = fq(u384 { limb0: 3, limb1: 0, limb2: 0, limb3: 0 }).neg(m).mul(p.x.sqr(m), m);
        // 2p.y
        let cy = p.y.add(p.y, m);
        sparse_fq12(
            p.y.mul(p.y, m).sub(fq(u384 { limb0: 9, limb1: 0, limb2: 0, limb3: 0 }), m),
            self.x.scale(cx.c0, m),
            self.y.scale(cy.c0, m)
        )
    }

    /// The sloped line function for adding two points
    #[inline(always)]
    fn at_chord(self: AffineG2, p1: AffineG1, p2: AffineG1, m: CircuitModulus) -> Fq12 {
        let cx = p2.y.sub(p1.y, m);
        let cy = p1.x.sub(p2.x, m);
        sparse_fq12(
            p1.y.mul(p2.x, m).sub(p2.y.mul(p1.x, m), m),
            self.x.scale(cx.c0, m),
            self.y.scale(cy.c0, m)
        )
    }
